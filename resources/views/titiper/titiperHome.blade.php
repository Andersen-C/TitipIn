...
@php
  ...
  $latestOrders = $latestOrders ?? collect([
    (object)['menu_name'=>'Nasi Goreng','status'=>'Pending','time'=>'14:22','note'=>'Sedang dibelikan'],
    ...
    <div class="grid lg:grid-cols-3 gap-8 items-start">
      ...
      <div class="lg:col-span-12">
        <div class="flex items-start gap-8">
          <div class="flex-1">
            <!-- Logo kecil -->
            <div class="mb-4">
              <span class="text-2xl font-extrabold text-sky-700">Titip<span class="text-yellow-400">In</span></span>
            </div>
            ...
            <div class="mt-6">
              <a href="{{ url('titip/create') }}" class="inline-block bg-sky-600 hover:bg-sky-700 text-white px-6 py-3 rounded-md shadow">Mulai Titip Sekarang</a>
            </div>
          </div>
        ...
        <div class="mt-8 grid md:grid-cols-2 gap-6">
          ...
          <div class="bg-white rounded-xl shadow p-5 border">
            <div class="flex items-center justify-between mb-4">
              <div>
                <h4 class="text-md font-semibold text-slate-800">Pesanan Terbaru <span class="text-sm text-slate-400">({{ $latestOrders->count() }})</span></h4>
            ...
            <div class="space-y-3">
              @foreach($latestOrders as $o)
                <div class="flex items-center justify-between">
                  <div>
                    <div class="text-sm font-medium">{{ $o->menu_name ?? ($o->menu->name ?? 'Menu') }}</div>
                    <div class="text-xs text-slate-500">{{ $o->note ?? '' }}</div>
                  </div>
                  <div class="text-right">
                    <div class="text-xs text-slate-400">{{ $o->time ?? optional($o->created_at)->format('H:i') }}</div>
                    <div class="mt-1">
                      @if( ($o->status ?? '') == 'Pending' || ($o->status ?? '') == 'pending')
                        <span class="px-2 py-1 rounded-full text-xs bg-yellow-100 text-yellow-800">Pending</span>
                      @else
                        <span class="px-2 py-1 rounded-full text-xs bg-slate-100 text-slate-700">{{ ucfirst($o->status ?? 'Selesai') }}</span>
                      @endif
                    </div>
                  </div>
                </div>
            ...
            <div class="mt-5 text-center">
              <a href="{{ url('orders') }}" class="inline-block px-4 py-2 border rounded-md text-sky-600">Lihat Detail</a>
            </div>
          </div>
      ...
      <aside class="space-y-12">
        <div class="bg-white rounded-xl shadow p-4">
            ...
            @foreach($recommended as $menu)
              <div class="bg-slate-50 rounded-xl p-3 shadow-sm flex items-center gap-3">
                <div class="w-20 h-20 bg-white rounded-lg overflow-hidden flex-shrink-0">
                  <img src="{{ $menu->image ?? 'https://via.placeholder.com/120' }}" alt="" class="w-full h-full object-cover">
                </div>
                <div class="flex-1">
                  <div class="font-medium text-sm">{{ $menu->name }}</div>
                  <div class="text-xs text-slate-500">Rp. {{ number_format($menu->price ?? 0,0,',','.') }}</div>
                  <div class="mt-2">
                    <a href="{{ url('menu/'.$menu->id ?? '#') }}" class="px-3 py-1 bg-sky-600 text-white rounded text-xs">Titip</a>
                  </div>
                </div>
              </div>
            @endforeach
          ...
          <div class="mt-4">
            <a href="{{ url('menu') }}" class="w-full block text-center bg-sky-600 text-white py-3 rounded-full">Selengkapnya</a>
          </div>
        </div>
      </aside>
    </div>
